...
const FamilyGroup = mongoose.models.FamilyGroup || mongoose.model('FamilyGroup', familyGroupSchema);
...
app.post('/api/register', authLimiter, async (req, res) => {
    try {
        ...
        if (!isValidUsername(username)) return res.status(400).json({ error: 'Username must be 3-30 characters (letters, numbers, _, -)' });
...
app.post('/api/transfer', authLimiter, authMiddleware, async (req, res) => {
    try {
        ...
        if (req.authUser !== from) return res.status(403).json({ error: 'Cannot send on behalf of another user' });
...
app.post('/api/user/:username/approve-request/:requestId', authMiddleware, ownerOnly, async (req, res) => {
    try {
        ...
        if (request.amount > (income - expenses)) return res.status(400).json({ error: 'Requester has insufficient balance' });
...
app.post('/api/family/create', authMiddleware, async (req, res) => {
    try {
        ...
        const group = await FamilyGroup.create({ name: name.trim(), code, ownerId: user._id, members: [username] });
...
app.get('/api/family/info', authMiddleware, async (req, res) => {
    try {
        ...
        const group = await FamilyGroup.findById(user.familyGroupId);
        if (!group) { user.familyGroupId = null; await user.save(); return res.json(null); }

        const isOwner = group.ownerId.toString() === user._id.toString();
        res.json({
            id: group._id, name: group.name, code: group.code, isOwner,
            memberCount: group.members.length, pendingCount: group.pendingTransactions.length
        });
    } catch (err) {
        console.error(err.message);
        res.status(500).json({ error: 'Server error' });
    ...
    try {
        ...
        for (const m of group.members) {
            ...
            if (memberUser) {
                ...
                const expenses = txs.filter(t => t.type === 'expense').reduce((s, t) => s + t.amount, 0);
                members.push({
                    username: m, balance: income - expenses,
...
app.get('/api/family/member/:memberName/transactions', authMiddleware, async (req, res) => {
    try {
        ...
        res.json({ username: memberUser.username, transactions: memberUser.transactions || [] });
    } catch (err) {
        console.error(err.message);
        res.status(500).json({ error: 'Server error' });
    }
...
app.post('/api/family/remove-member', authMiddleware, async (req, res) => {
    try {
        ...
        if (removedUser) { removedUser.familyGroupId = null; await removedUser.save(); }
        res.json({ success: true });
    } catch (err) {
        console.error(err.message);
        res.status(500).json({ error: 'Server error' });
...
app.post('/api/family/leave', authMiddleware, async (req, res) => {
    try {
        ...
        const group = await FamilyGroup.findById(user.familyGroupId);
        if (!group) { user.familyGroupId = null; await user.save(); return res.json({ success: true }); }
        ...
        if (isOwner && group.members.length === 0) await FamilyGroup.findByIdAndDelete(group._id);
        res.json({ success: true });
    } catch (err) {
        console.error(err.message);
        res.status(500).json({ error: 'Server error' });
...
app.post('/api/user/:username/2fa/login', async (req, res) => {
    try {
        ...
        if (!decoded.temp || decoded.username !== req.params.username) return res.status(403).json({ error: 'Invalid token for 2FA' });
...
app.get('/backup', (req, res) => res.sendFile(path.join(__dirname, '..', 'public', 'backup.html')));
